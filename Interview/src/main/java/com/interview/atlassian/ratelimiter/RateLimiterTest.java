...
/**
 * @author aayush https://medium.com/@aayushbhatnagar_10462/rate-limiting-implementation-example-in-java-7831923e5de3
 * This is a simple test for a rate limiter.
 * We can build the RateLimitExecutor object and set the following
 * attributes:
 * a) instance_id — which acts as a key to uniquely identify the rate limiting policy
 * b) threshold — the Transactions per second to be controlled (throttle limit)
 * <p>
 * Based on this information, provision the rate limiter in a container class — Rate Limit Manager
 * This class then pumps loads through iterations, and sleep time to control traffic ingestion.
 */
public class RateLimiterTest implements RateLimitListener {
    ...
    private void test() throws InterruptedException {
        ...
        rateLimiter.build(TimeUnit.SECONDS, threshold_to_be_enforced);   // Set the thresholds / second.
        rateLimiter.setInstance_id(instance_id);   // Associate instance ID
        RateLimitManager._instance.provisionRateLimit(rateLimiter, instance_id, new RateLimitThrottleListener());// Provision Rate Limit
